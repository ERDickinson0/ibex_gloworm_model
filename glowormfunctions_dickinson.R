##################### GloWorm Function - ERD ########################################
###### Writing to gloworm function to make it easier to run. ########################
##################### April 2020 ####################################################
...
#g.interp <- approxfun(y = grazing_plots, 
  #                    x = c(which(as.character(date.range) 
 #                                 %in% as.character(movement_dates))), 
   #                   method = 'constant')
